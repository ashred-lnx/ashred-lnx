						 ...
						 struct ethhdr *ethh)
{
	...
	if (es_vtep)
		nh = IS_IPADDR_V4(&es_vtep->vtep_ip) ? es_vtep->vtep_ip.ipaddr_v4
						      : (struct in_addr){0};
	else
		nh.s_addr = 0;
...
static void zebra_evpn_arp_nd_read(struct event *t)
{
	...
	for (count = 0; count < ZEBRA_EVPN_ARP_ND_PKT_MAX; ++count) {
		if (zebra_evpn_arp_nd_recvmsg(fd, buf, sizeof(buf), &vlan, &len, &errno_ret) < 0) {
			...
			if (errno_ret == EWOULDBLOCK || errno_ret == EAGAIN)
				break;

			if (IS_ZEBRA_DEBUG_EVPN_MH_ARP_ND_PKT)
				zlog_debug("evpn arp_nd read failed:len %d %s", len,
					   safe_strerror(errno_ret));
			break;
		}
